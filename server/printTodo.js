...
async function generatePDF(todo_id) {
    return new Promise(async (resolve, reject) => {
        try {
            const PDFDocument = require("pdfkit");
            const fs = require("fs");

            // FIX: Ensure todo_id is a number
            todo_id = Number(todo_id);
            if (isNaN(todo_id)) {
                console.error("Invalid todo_id received:", todo_id);
                return reject("Invalid todo_id");
            }
            ...
            const todo = await pool.query(
                ...
                LEFT JOIN users u2 ON t.updated_by = u2.user_id
                WHERE t.todo_id = $1`,
                [todo_id]  // Now always a valid number
            );

            if (todo.rows.length === 0) return reject("Todo not found");
